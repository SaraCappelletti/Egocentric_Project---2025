...
def get_model_class(model_name):
    ...
    if model_name == "vslnet":
        from model.VSLNet import VSLNet
        return VSLNet
    elif model_name == "vslbase":
...
def main(configs, parser):
    ...
    if configs.mode.lower() == "train":
        ...
        # build model (new)
        ModelClass = get_model_class(configs.model_name)
        model = ModelClass(configs=configs, word_vectors=dataset.get("word_vector", None)).to(device)
        ...
        for epoch in range(configs.epochs):
            ...
            for data in tqdm(
                ...
                # compute logits and loss (new)
                output = model(word_ids, char_ids, vfeats, video_mask, query_mask)
                if isinstance(model, VSLNet):
                    h_score, start_logits, end_logits = output
                    ...
                    highlight_loss = torch.tensor(0.0, device=device)  # dummy
                    loc_loss = model.compute_loss(
                      start_logits, end_logits, s_labels, e_labels
                    )
                    total_loss = loc_loss  # no highlight loss
                ...
                if writer is not None and global_step % configs.tb_log_freq == 0:
                  ...
                  if configs.model_name.lower() == "vslnet":
                      writer.add_scalar("Loss/Highlight", highlight_loss.item(), global_step)
                      writer.add_scalar("Loss/Highlight (*lambda)", (configs.highlight_lambda * highlight_loss.item()), global_step)
